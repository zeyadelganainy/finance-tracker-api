      ...
      <div>
        ...
        <div className="mb-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="font-display text-3xl text-ink">{t('transactions.title')}</h1>
          ...
          <Card className="mb-6">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="eyebrow">{t('transactions.filters')}</h3>
                {hasFilters && (
...
function TransactionRow({
  ...
  if (isEditing) {
    const editInputClass =
      'w-full rounded-md border border-line-strong bg-app-surface px-3 py-1.5 text-sm text-ink focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent';
    return (
      <tr className="bg-accent-soft">
        <td className="px-4 py-4">
          <input type="checkbox" className="h-4 w-4 rounded border-line-strong accent-[var(--accent-color)]" checked={selected} onChange={onToggleSelect} />
        </td>
  ...
  return (
    <tr className="transition-colors hover:bg-app-elevated">
      <td className="px-4 py-4">
        <input
          type="checkbox"
          className="h-4 w-4 rounded border-line-strong accent-[var(--accent-color)]"
      ...
      <td className="px-6 py-4 text-sm text-ink">
        {transaction.description || <span className="italic text-ink-faint">{t('transactions.noDescription')}</span>}
      </td>
      ...
      <td className={`whitespace-nowrap px-6 py-4 text-right font-mono text-sm font-semibold tnum ${amountColor}`}>
          ...
          <Card key={transaction.id} className="p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-sm text-ink-muted">{format(parseTxnDate(transaction.date), 'MMM dd, yyyy')}</span>
              </div>
              <span className={`text-right font-mono text-lg font-semibold tnum ${amountColor}`}>
                {formatCurrency(transaction.amount)}
              </span>
            </div>

            <div className="mt-2 line-clamp-2 text-sm text-ink">
              {transaction.description ? (
                transaction.description
              ) : (
                <span className="italic text-ink-faint">{t('transactions.noDescription')}</span>
              )}
            </div>

            <div className="mt-3 flex items-center justify-between gap-2">
              <Badge variant="default">{transaction.category.name}</Badge>
              <span className={`rounded-full border border-line px-2 py-1 text-xs ${isExpense ? 'text-danger' : 'text-success'}`}>
  ...
  return (
    <Modal isOpen onClose={onClose} title={t('transactions.editTitle')} size="md">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        />
        <div>
          <label className="mb-1.5 block text-xs font-medium text-ink-muted">{t('transactions.description')}</label>
          <textarea
  ...
  return (
    <Modal isOpen onClose={onClose} title={t('transactions.new')} size="md">
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {formData.amount && (
          <div className="rounded-md border border-line bg-app-elevated p-3">
            <div className={`flex items-center gap-2 ${isExpense ? 'text-danger' : 'text-success'}`}>
              {isExpense ? (
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <span className="font-mono text-sm font-medium tnum">
                {isExpense ? t('transactions.expense') : t('transactions.income')}: {formatCurrency(parseFloat(formData.amount))}
              </span>
            </div>
          </div>
        )}
