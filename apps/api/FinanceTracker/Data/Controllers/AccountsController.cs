...
public class AccountsController : ControllerBase
{
    ...
    public AccountsController(AppDbContext db, ICurrentUserContext currentUser, IOFXParserService ofxParser)
    {
        _db = db;
    ...
    // OFX/QFX Import Endpoints
    [HttpPost("{id}/transactions/import/ofx")]
    public async Task<IActionResult> ImportOFXPreview(Guid id, IFormFile file)
    {
        ...
        if (fileExt != ".ofx" && fileExt != ".qfx")
            return BadRequest(new { error = "Only .ofx and .qfx files are supported" });
        ...
        {
            // Parse OFX file
            var rows = await _ofxParser.ParseOFXAsync(file.OpenReadStream());

            // Convert to response format
            var responseRows = rows.Select(r => new ImportTransactionRow(
            ...
            return Ok(new OFXParseResponse(responseRows));
        }
        catch (Exception ex)
        {
            return BadRequest(new { error = $"Failed to parse OFX file: {ex.Message}" });
        }
    }

    [HttpPost("{id}/transactions/import/ofx/confirm")]
    public async Task<IActionResult> ConfirmOFXImport(Guid id, OFXConfirmRequest req)
    {
        var userId = Guid.Parse(_currentUser.UserId);
        ...
        var skipped = 0;
        var failed = 0;
        var errors = new List<OFXImportError>();
        ...
        for (var i = 0; i < req.Transactions.Count; i++)
        {
            ...
            try
            {
                ...
                {
                    failed++;
                    errors.Add(new OFXImportError(rowNum, $"Invalid date format: {dto.Date}"));
                    continue;
                }
                ...
                {
                    failed++;
                    errors.Add(new OFXImportError(rowNum, "Amount cannot be 0"));
                    continue;
                }
                ...
                // Default to "Uncategorized" if not provided
                if (categoryId == null)
                {
                    var uncategorized = await _db.Categories
                        .FirstOrDefaultAsync(c => c.UserId == userId && c.Name == "Uncategorized");
            ...
            {
                failed++;
                errors.Add(new OFXImportError(rowNum, ex.Message));
            }
        }
